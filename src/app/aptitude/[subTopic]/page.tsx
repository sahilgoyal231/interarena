...
import { Clock, CircleMinus, AlertCircle, ArrowRight, ArrowLeft, Lightbulb, CheckCircle2, XCircle, LayoutGrid, ChevronDown, ChevronUp } from "lucide-react";
    ...
    async function loadQuestions() {
      try {
        ...
        if (res.ok) {
          ...
          const limit = subTopic === "Mix Practice" ? Math.floor(defaultTime / 90) : 20;
      ...
      <div className="h-screen bg-zinc-950 text-zinc-100 font-sans overflow-hidden flex flex-col">
        <div className="flex-1 overflow-y-auto p-6 md:p-12" data-lenis-prevent="true">
          ...
          <div className="text-center space-y-4 mb-12">
            ...
            <p className="text-zinc-400">Review your performance diagnostics for <span className="text-purple-400 font-bold">{subTopic}</span></p>
            ...
            <div className="flex flex-wrap justify-center gap-4 md:gap-6 mt-8">
              <div className="bg-zinc-900/50 border border-zinc-800 p-6 rounded-2xl w-36 md:w-40 flex flex-col items-center">
                <p className="text-3xl md:text-4xl font-black text-purple-400">{score}<span className="text-lg md:text-xl text-zinc-500">/{questions.length}</span></p>
                <p className="text-[10px] md:text-xs uppercase tracking-widest text-zinc-500 mt-2 text-center">Score</p>
              </div>
              <div className="bg-zinc-900/50 border border-zinc-800 p-6 rounded-2xl w-36 md:w-40 flex flex-col items-center">
                <p className={`text-3xl md:text-4xl font-black ${accuracy >= 70 ? 'text-green-400' : accuracy >= 40 ? 'text-amber-400' : 'text-rose-400'}`}>{accuracy}%</p>
                <p className="text-[10px] md:text-xs uppercase tracking-widest text-zinc-500 mt-2 text-center">Accuracy</p>
              </div>
              <div className="bg-zinc-900/50 border border-zinc-800 p-6 rounded-2xl w-36 md:w-40 flex flex-col items-center">
                <p className="text-3xl md:text-4xl font-black text-zinc-400">{unansweredCount}</p>
                <p className="text-[10px] md:text-xs uppercase tracking-widest text-zinc-500 mt-2 text-center">Skipped</p>
              </div>
            </div>
          </div>

          <div className="space-y-6">
            <h3 className="text-xl font-bold text-white border-b border-zinc-800 pb-4">Detailed Review</h3>
            {questions.map((q, index) => {
              ...
              return (
                <Card key={q.id} className={`bg-zinc-900/30 border-zinc-800/80 rounded-2xl p-6 space-y-4 ${isUnanswered ? 'opacity-80' : ''}`}>
                  <div className="flex items-start justify-between gap-4">
                    <p className="text-zinc-200 font-medium"><span className="text-purple-500 font-bold mr-2">Q{index + 1}.</span>{q.prompt}</p>
                  ...
                  <div className="p-4 rounded-xl bg-zinc-950 border border-zinc-800 space-y-2 text-sm">
                    ...
                    {isUnanswered ? (
                      <p><span className="text-zinc-500">Your Answer:</span> <span className="text-zinc-500 font-semibold italic">Not Answered</span></p>
                    ) : (
                      <p><span className="text-zinc-500">Your Answer:</span> <span className={isCorrect ? "text-green-400 font-semibold" : "text-rose-400 font-semibold"}>{userAnswer}</span></p>
                    )}
                    
                    {/* Always show the correct answer if they got it wrong or skipped it */}
                    {!isCorrect && <p><span className="text-zinc-500">Correct Answer:</span> <span className="text-green-400 font-semibold">{q.correctAnswer}</span></p>}
                  </div>
          ...
          <div className="flex justify-center pt-8">
            <button onClick={() => router.push('/aptitude')} className="px-8 py-3 bg-zinc-100 text-zinc-950 font-bold uppercase tracking-widest rounded-xl hover:bg-zinc-300 transition-colors shadow-[0_0_15px_rgba(255,255,255,0.1)]">
              Return to Dashboard
            </button>
          </div>
        </div>
  ...
  return (
    <div className="h-screen bg-zinc-950 text-zinc-100 font-sans flex flex-col overflow-hidden">
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div ref={scrollContainerRef} className="flex-1 overflow-y-auto p-6 md:p-12 relative" data-lenis-prevent="true">
        ...
        <div className="w-80 border-l border-zinc-800 bg-zinc-950 flex flex-col shrink-0 hidden lg:flex overflow-hidden">
